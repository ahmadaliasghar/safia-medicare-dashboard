...
const Sidebar = () => {
  ...
  return (
    <>
<nav className="fixed top-0 z-50 w-full bg-[#003049] border-b text-white border-gray-200">
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center">
          <div className="flex items-center ms-3">
            <div>
              <button type="button" className="flex text-sm  bg-white rounded-full focus:ring-4 focus:ring-gray-300" aria-expanded="false" data-dropdown-toggle="dropdown-user">
...
<aside id="logo-sidebar" className=" hidden lg:block fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-[#003049] border-r text-white border-gray-200 sm:translate-x-0 " aria-label="Sidebar">
   <div className="h-full px-3 pb-4 overflow-y-auto bg-[#003049] text-white">
      <ul className="space-y-2 font-medium">
        {links.map((link, index) => (
            <li key={index}>
            <Link href={link.path} className="flex items-center p-2 text-white rounded-r-lg hover:border-l-6 border-l-4 hover:border-white hover:bg-[#012350ff] hover:text-white group">
               {link.icon}
